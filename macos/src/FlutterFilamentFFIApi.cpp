...
int get_hierarchy_entity_count_ffi(void *const assetManager, EntityId asset) {
  std::packaged_task<int()> lambda([&] { return get_hierarchy_entity_count(assetManager, asset); });
  auto fut = _rl->add_task(lambda);
  fut.wait();
  return fut.get();
}
...
FLUTTER_PLUGIN_EXPORT
void get_hierarchy_entity_name_ffi(void *const assetManager, EntityId asset, char *const outPtr, int index) {
  std::packaged_task<void()> lambda([&] { get_hierarchy_entity_name(assetManager, asset, outPtr, index); });
  auto fut = _rl->add_task(lambda);
  fut.wait();
}
}
