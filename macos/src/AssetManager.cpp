...
size_t AssetManager::getHierarchyEntityCount(EntityId entity) const noexcept {
    const auto& pos = _entityIdLookup.find(entity);
    if(pos == _entityIdLookup.end()) {
        Log("ERROR: asset not found for entity.");
        return 0;
    ...
    auto& asset = _assets[pos->second];
    return asset.mAsset->getEntityCount();
}

const char* AssetManager::getHierarchyEntityName(EntityId entity, int index)  {
    const auto& pos = _entityIdLookup.find(entity);
    if(pos == _entityIdLookup.end()) {
        Log("ERROR: asset not found for entity.");
        return nullptr;
