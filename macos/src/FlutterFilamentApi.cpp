...
#include "FlutterFilamentApi.h"
    ...
    int get_hierarchy_entity_count(void *assetManager, EntityId asset)
    {
        return (int)((AssetManager *)assetManager)->getHierarchyEntityCount(asset);
    }

    FLUTTER_PLUGIN_EXPORT void get_hierarchy_entity_name(void *assetManager, EntityId asset, char *const outPtr, int index)
    {
        string name = ((AssetManager *)assetManager)->getHierarchyEntityName(asset, index);
        strcpy(outPtr, name.c_str());
    }
